...
log.inAutoFile = function (preferences, suffix) {
	...
	fs.mkdir(path.resolve(__dirname, logDir));	//SOMETIMES IT DOES NOT WORK DUE TO PERMISSION ISSUES!
	console.log("logPath = " + path.resolve(__dirname, logDir));
	fs.writeFileSync(path.resolve(__dirname, (logDir + "/" + curDate + "_" + suffix + ".json")), JSON.stringify(preferences, null, 4));
